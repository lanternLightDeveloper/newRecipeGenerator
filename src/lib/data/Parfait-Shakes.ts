// Current Recipe Count: 30

export const ParfaitShakes: Recipe[] = [
	{
		id: 1,
		name: 'Classic Yogurt Parfait',
		servings: '2',
		ingredients: [
			'1 cup vanilla Greek yogurt',
			'½ cup granola',
			'½ cup mixed berries (strawberries, blueberries, raspberries)',
			'1 tsp honey'
		],
		instructions: [
			'Spoon a layer of yogurt into serving glasses.',
			'Add granola, then berries.',
			'Repeat layers and drizzle with honey.',
			'Serve chilled.'
		],
		tags: [
			'parfait',
			'yogurt',
			'berries',
			'breakfast',
			'vegetarian',
			'easy',
			'no-cook',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '290 calories, 40g carbs, 7g fat, 15g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 2,
		name: 'Tropical Mango Coconut Parfait',
		servings: '2',
		ingredients: [
			'1 cup coconut yogurt',
			'½ cup diced mango',
			'2 tbsp shredded coconut',
			'¼ cup granola'
		],
		instructions: [
			'Layer yogurt, mango, and granola in glasses.',
			'Sprinkle shredded coconut on top.',
			'Chill 10 minutes before serving.'
		],
		tags: [
			'parfait',
			'mango',
			'tropical',
			'coconut',
			'gluten-free',
			'vegan',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '260 calories, 32g carbs, 10g fat, 6g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 3,
		name: 'Peach Cobbler Parfait',
		servings: '2',
		ingredients: [
			'1 cup vanilla yogurt',
			'½ cup diced peaches',
			'¼ cup crushed graham crackers',
			'1 tsp cinnamon',
			'1 tsp honey'
		],
		instructions: [
			'Layer yogurt, peaches, and graham crackers.',
			'Top with cinnamon and honey drizzle.',
			'Serve immediately or chill.'
		],
		tags: [
			'parfait',
			'peach',
			'dessert-style',
			'sweet',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '310 calories, 42g carbs, 9g fat, 12g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 4,
		name: 'Strawberry Shortcake Parfait',
		servings: '2',
		ingredients: [
			'1 cup vanilla yogurt',
			'½ cup sliced strawberries',
			'¼ cup pound cake cubes',
			'1 tbsp whipped cream'
		],
		instructions: [
			'Layer yogurt, strawberries, and pound cake pieces.',
			'Top with whipped cream before serving.'
		],
		tags: [
			'parfait',
			'strawberry',
			'dessert-style',
			'sweet',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '340 calories, 45g carbs, 10g fat, 13g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 5,
		name: 'Apple Cinnamon Parfait',
		servings: '2',
		ingredients: [
			'1 cup plain yogurt',
			'½ cup cinnamon-spiced cooked apples',
			'¼ cup granola',
			'1 tsp maple syrup'
		],
		instructions: [
			'Layer yogurt, cooked apples, and granola.',
			'Drizzle with maple syrup and serve warm or chilled.'
		],
		tags: [
			'parfait',
			'apple',
			'cinnamon',
			'cozy',
			'fall',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 38g carbs, 9g fat, 11g protein',
		time: 12,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 6,
		name: 'Blueberry Lemon Parfait',
		servings: '2',
		ingredients: [
			'1 cup lemon yogurt',
			'½ cup fresh blueberries',
			'¼ cup granola',
			'1 tsp lemon zest'
		],
		instructions: [
			'Layer yogurt, blueberries, and granola.',
			'Top with lemon zest and chill before serving.'
		],
		tags: [
			'parfait',
			'blueberry',
			'lemon',
			'fresh',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '260 calories, 36g carbs, 6g fat, 12g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 7,
		name: 'Mocha Coffee Parfait',
		servings: '2',
		ingredients: [
			'1 cup coffee-flavored Greek yogurt',
			'¼ cup crushed chocolate wafers',
			'1 tbsp cocoa nibs',
			'½ tsp espresso powder'
		],
		instructions: [
			'Layer yogurt and crushed wafers.',
			'Sprinkle with cocoa nibs and espresso powder.',
			'Serve chilled for a morning caffeine boost.'
		],
		tags: [
			'parfait',
			'coffee',
			'mocha',
			'energizing',
			'dessert-style',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '310 calories, 34g carbs, 8g fat, 14g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 8,
		name: 'Banana Nut Parfait',
		servings: '2',
		ingredients: [
			'1 cup Greek yogurt',
			'1 sliced banana',
			'2 tbsp chopped walnuts',
			'1 tbsp honey'
		],
		instructions: [
			'Layer yogurt and banana slices in a glass.',
			'Top with walnuts and drizzle with honey.'
		],
		tags: [
			'parfait',
			'banana',
			'nuts',
			'protein-rich',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '330 calories, 35g carbs, 12g fat, 16g protein',
		time: 8,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 9,
		name: 'Berry Almond Parfait',
		servings: '2',
		ingredients: [
			'1 cup almond yogurt',
			'½ cup mixed berries',
			'2 tbsp sliced almonds',
			'1 tsp agave syrup'
		],
		instructions: ['Layer yogurt, berries, and almonds.', 'Finish with a drizzle of agave syrup.'],
		tags: [
			'parfait',
			'berry',
			'almond',
			'vegan',
			'gluten-free',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '250 calories, 28g carbs, 9g fat, 9g protein',
		time: 8,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 10,
		name: 'Pumpkin Pie Parfait',
		servings: '2',
		ingredients: [
			'1 cup pumpkin puree',
			'½ cup vanilla yogurt',
			'¼ cup crushed graham crackers',
			'½ tsp pumpkin pie spice'
		],
		instructions: [
			'Layer pumpkin puree, yogurt, and graham crackers.',
			'Sprinkle with pumpkin pie spice and chill.'
		],
		tags: [
			'parfait',
			'pumpkin',
			'fall',
			'spiced',
			'dessert-style',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 42g carbs, 8g fat, 10g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 11,
		name: 'Peanut Butter Chocolate Parfait',
		servings: '2',
		ingredients: [
			'1 cup chocolate yogurt',
			'2 tbsp peanut butter',
			'¼ cup crushed pretzels',
			'1 tbsp chocolate chips'
		],
		instructions: [
			'Layer chocolate yogurt with peanut butter swirls.',
			'Top with pretzels and chocolate chips.'
		],
		tags: [
			'parfait',
			'chocolate',
			'peanut butter',
			'sweet',
			'dessert-style',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '380 calories, 34g carbs, 18g fat, 15g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 12,
		name: 'Matcha Green Tea Parfait',
		servings: '2',
		ingredients: [
			'1 cup vanilla yogurt',
			'1 tsp matcha powder',
			'¼ cup granola',
			'2 tbsp kiwi slices'
		],
		instructions: [
			'Whisk matcha into yogurt until smooth.',
			'Layer yogurt, granola, and kiwi slices.'
		],
		tags: [
			'parfait',
			'matcha',
			'green tea',
			'energizing',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '240 calories, 30g carbs, 7g fat, 10g protein',
		time: 8,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 13,
		name: 'Maple Pecan Parfait',
		servings: '2',
		ingredients: [
			'1 cup maple-flavored yogurt',
			'¼ cup chopped pecans',
			'¼ cup granola',
			'1 tsp maple syrup'
		],
		instructions: ['Layer yogurt, granola, and pecans.', 'Drizzle with extra maple syrup.'],
		tags: [
			'parfait',
			'maple',
			'pecan',
			'sweet',
			'cozy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '310 calories, 35g carbs, 11g fat, 12g protein',
		time: 8,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 14,
		name: 'Raspberry White Chocolate Parfait',
		servings: '2',
		ingredients: [
			'1 cup vanilla yogurt',
			'½ cup raspberries',
			'2 tbsp white chocolate chips',
			'¼ cup granola'
		],
		instructions: ['Layer yogurt, raspberries, and granola.', 'Top with white chocolate chips.'],
		tags: [
			'parfait',
			'raspberry',
			'white chocolate',
			'sweet',
			'dessert-style',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '330 calories, 40g carbs, 10g fat, 13g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 15,
		name: 'Cherry Cheesecake Parfait',
		servings: '2',
		ingredients: [
			'1 cup cream cheese yogurt',
			'½ cup cherry pie filling',
			'¼ cup crushed graham crackers'
		],
		instructions: [
			'Layer yogurt, cherries, and graham crackers.',
			'Chill 30 minutes before serving for best flavor.'
		],
		tags: [
			'parfait',
			'cherry',
			'cheesecake',
			'dessert-style',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '370 calories, 44g carbs, 12g fat, 14g protein',
		time: 10,
		creator: 'Generated',
		category: 'Blended Drinks'
	},

	{
		id: 16,
		name: 'Strawberry Banana Protein Shake',
		servings: '2',
		ingredients: [
			'1 cup strawberries, fresh or frozen',
			'1 banana',
			'1 cup milk (or almond milk)',
			'½ cup Greek yogurt',
			'1 scoop vanilla protein powder',
			'1 tsp honey (optional)'
		],
		instructions: [
			'Combine all ingredients in a blender.',
			'Blend until smooth and creamy.',
			'Pour into glasses and serve immediately.'
		],
		tags: [
			'shake',
			'strawberry',
			'banana',
			'protein',
			'breakfast',
			'smoothie',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 40g carbs, 6g fat, 22g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 17,
		name: 'Mango Pineapple Tropical Shake',
		servings: '2',
		ingredients: [
			'1 cup diced mango',
			'1 cup diced pineapple',
			'1 cup coconut milk',
			'½ cup Greek yogurt',
			'1 tbsp chia seeds'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Pour into glasses and garnish with extra chia seeds if desired.'
		],
		tags: [
			'shake',
			'tropical',
			'mango',
			'pineapple',
			'healthy',
			'breakfast',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '250 calories, 38g carbs, 8g fat, 8g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 18,
		name: 'Chocolate Peanut Butter Breakfast Shake',
		servings: '2',
		ingredients: [
			'1 banana',
			'1 cup milk',
			'2 tbsp peanut butter',
			'1 tbsp cocoa powder',
			'½ cup Greek yogurt',
			'1 tsp honey'
		],
		instructions: [
			'Combine all ingredients in a blender.',
			'Blend until smooth and creamy.',
			'Serve immediately.'
		],
		tags: [
			'shake',
			'chocolate',
			'peanut butter',
			'protein',
			'sweet',
			'breakfast',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '350 calories, 35g carbs, 14g fat, 20g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 19,
		name: 'Blueberry Almond Breakfast Shake',
		servings: '2',
		ingredients: [
			'1 cup blueberries',
			'1 cup almond milk',
			'½ cup Greek yogurt',
			'1 tbsp almond butter',
			'1 tsp honey (optional)'
		],
		instructions: ['Blend all ingredients until smooth.', 'Pour into glasses and serve chilled.'],
		tags: [
			'shake',
			'blueberry',
			'almond',
			'breakfast',
			'antioxidants',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '260 calories, 32g carbs, 9g fat, 12g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 20,
		name: 'Green Spinach & Avocado Shake',
		servings: '2',
		ingredients: [
			'1 cup fresh spinach',
			'½ avocado',
			'1 banana',
			'1 cup almond milk',
			'½ cup Greek yogurt',
			'1 tsp honey'
		],
		instructions: [
			'Blend all ingredients until creamy and smooth.',
			'Serve immediately for a fresh green breakfast boost.'
		],
		tags: [
			'shake',
			'green',
			'spinach',
			'avocado',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 34g carbs, 10g fat, 12g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 21,
		name: 'Tropical Papaya Smoothie',
		servings: '2',
		ingredients: [
			'1 cup papaya, diced',
			'1 banana',
			'1 cup orange juice',
			'½ cup Greek yogurt',
			'1 tsp flax seeds'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Serve chilled, garnished with a few papaya cubes.'
		],
		tags: [
			'shake',
			'papaya',
			'tropical',
			'breakfast',
			'fiber-rich',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '240 calories, 38g carbs, 5g fat, 10g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 22,
		name: 'Cinnamon Apple Breakfast Shake',
		servings: '2',
		ingredients: [
			'1 apple, cored and diced',
			'1 cup milk',
			'½ cup Greek yogurt',
			'½ tsp cinnamon',
			'1 tsp honey',
			'1 scoop vanilla protein powder'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Serve immediately for a warm cinnamon flavor.'
		],
		tags: [
			'shake',
			'apple',
			'cinnamon',
			'protein',
			'breakfast',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '290 calories, 38g carbs, 7g fat, 20g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 23,
		name: 'Raspberry Vanilla Breakfast Shake',
		servings: '2',
		ingredients: [
			'1 cup raspberries',
			'1 cup milk',
			'½ cup vanilla Greek yogurt',
			'1 tsp honey',
			'½ tsp vanilla extract'
		],
		instructions: [
			'Combine all ingredients in a blender.',
			'Blend until smooth and creamy.',
			'Serve immediately.'
		],
		tags: [
			'shake',
			'raspberry',
			'vanilla',
			'breakfast',
			'antioxidants',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '260 calories, 35g carbs, 6g fat, 12g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 24,
		name: 'Mocha Coffee Breakfast Shake',
		servings: '2',
		ingredients: [
			'1 cup brewed coffee, cooled',
			'1 banana',
			'½ cup milk',
			'½ cup Greek yogurt',
			'1 tsp cocoa powder',
			'1 tsp honey'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Serve immediately for a morning caffeine boost.'
		],
		tags: [
			'shake',
			'coffee',
			'chocolate',
			'energizing',
			'breakfast',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 38g carbs, 5g fat, 15g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 25,
		name: 'Peanut Butter Banana Shake',
		servings: '2',
		ingredients: [
			'1 banana',
			'1 cup milk',
			'2 tbsp peanut butter',
			'½ cup Greek yogurt',
			'1 tsp honey'
		],
		instructions: ['Blend all ingredients until creamy and smooth.', 'Serve immediately.'],
		tags: [
			'shake',
			'peanut butter',
			'banana',
			'protein',
			'breakfast',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '330 calories, 34g carbs, 12g fat, 20g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 26,
		name: 'Kiwi Spinach Breakfast Shake',
		servings: '2',
		ingredients: [
			'2 kiwis, peeled',
			'1 cup fresh spinach',
			'1 banana',
			'1 cup almond milk',
			'½ cup Greek yogurt'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Serve chilled for a fresh, green breakfast start.'
		],
		tags: [
			'shake',
			'kiwi',
			'green',
			'spinach',
			'healthy',
			'breakfast',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '250 calories, 36g carbs, 5g fat, 12g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 27,
		name: 'Mixed Berry Protein Shake',
		servings: '2',
		ingredients: [
			'½ cup blueberries',
			'½ cup raspberries',
			'½ cup strawberries',
			'1 cup milk',
			'½ cup Greek yogurt',
			'1 scoop vanilla protein powder'
		],
		instructions: [
			'Blend all berries with milk, yogurt, and protein powder until smooth.',
			'Serve immediately.'
		],
		tags: [
			'shake',
			'berry',
			'protein',
			'antioxidants',
			'breakfast',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 38g carbs, 6g fat, 22g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 28,
		name: 'Pumpkin Spice Breakfast Shake',
		servings: '2',
		ingredients: [
			'½ cup pumpkin puree',
			'1 cup milk',
			'½ cup Greek yogurt',
			'½ tsp pumpkin pie spice',
			'1 tsp honey'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Serve chilled with a sprinkle of pumpkin pie spice.'
		],
		tags: [
			'shake',
			'pumpkin',
			'spice',
			'fall',
			'breakfast',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '240 calories, 30g carbs, 5g fat, 12g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 29,
		name: 'Chocolate Mint Breakfast Shake',
		servings: '2',
		ingredients: [
			'1 banana',
			'1 cup milk',
			'½ cup Greek yogurt',
			'1 tbsp cocoa powder',
			'2-3 fresh mint leaves',
			'1 tsp honey'
		],
		instructions: [
			'Blend all ingredients until smooth.',
			'Serve immediately for a refreshing chocolate-mint start.'
		],
		tags: [
			'shake',
			'chocolate',
			'mint',
			'breakfast',
			'protein',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '280 calories, 32g carbs, 6g fat, 15g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	},
	{
		id: 30,
		name: 'Orange Creamsicle Breakfast Shake',
		servings: '2',
		ingredients: ['1 cup orange juice', '½ cup vanilla Greek yogurt', '½ cup milk', '1 tsp honey'],
		instructions: ['Blend all ingredients until smooth and creamy.', 'Serve immediately chilled.'],
		tags: [
			'shake',
			'orange',
			'citrus',
			'breakfast',
			'creamsicle',
			'healthy',
			'vegetarian',
			'economic',
			'fast',
			'easy',
			'gluten-free'
		],
		nutrition: '220 calories, 36g carbs, 5g fat, 10g protein',
		time: 5,
		creator: 'Generated',
		category: 'Blended Drinks'
	}
];
